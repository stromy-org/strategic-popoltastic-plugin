...
**CRITICAL — Aspect ratio**: The `cx` (width) and `cy` (height) values are independent EMU numbers. Always derive one from the other using the source image's actual aspect ratio. Never set both to arbitrary values — this causes visible stretching/squashing.
...
# Option A: constrain by width
cx = 3000000  # desired width in EMUs
cy = int(cx / aspect_ratio)

# Option B: fit within a bounding box
max_cx, max_cy = 3000000, 2000000
cx = max_cx
cy = int(cx / aspect_ratio)
if cy > max_cy:
    cy = max_cy
    cx = int(cy * aspect_ratio)
```
...
<p:pic>
  <p:nvPicPr>
    ...
    <p:cNvPicPr>
      <!-- noChangeAspect="1" prevents UI-level distortion in PowerPoint -->
  ...
  <p:spPr>
    <a:xfrm>
      <a:off x="1000000" y="1000000"/>
      <!-- cx and cy MUST maintain the source image's aspect ratio -->
      <a:ext cx="3000000" cy="2000000"/>
    </a:xfrm>
    <a:prstGeom prst="rect">
      <a:avLst/>
