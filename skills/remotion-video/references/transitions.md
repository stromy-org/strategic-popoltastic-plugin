...
## Known bug: Studio crash with `props.stack` (Remotion ≤ 4.0.424)

Remotion's Studio-only createElement proxy crashes with `Cannot read properties of null (reading 'stack')` when a registered component receives `null` props. This only affects the Studio preview — CLI renders work fine.

**Fix:** Patch both copies of the proxy in `node_modules/remotion/dist/`:

```bash
# ESM bundle (dist/esm/index.mjs) — change `props.stack` to `props?.stack`
sed -i '' 's/const newProps = props\.stack/const newProps = props\?.stack/' \
  node_modules/remotion/dist/esm/index.mjs

# CJS bundle (dist/cjs/enable-sequence-stack-traces.js) — same fix
...
After patching, clear the webpack cache and restart the Studio:
...
**Important:** This patch is lost on `npm install`. Re-apply after installing dependencies, or add it as a `postinstall` script.
